...
`conservative-fade-v6` adds repeated animated bumper evidence within an episode. A qualifying transition needs a fade, at least 800 ms overlapping silence, 500 ms to the configured maximum black duration, complete context, and no explicit exclusion. Nine pre-fade frames spanning four seconds must each match the corresponding frame at two other transitions, with all three spaced at least three minutes apart (or the configured minimum spacing, whichever is larger). Mean absolute pixel difference is bounded by `bumperVisualDifference` (default 0.03). Texture and motion requirements reject blank or static templates. More than 256 eligible templates disables this optional comparison to bound pairwise work.

A matching bumper can replace the generic surrounding-audio, scene-difference, motion, and nearby-darkness vetoes, since commercial bumpers may return to the same quiet scene. It cannot override missing fades/silence, duration limits, incomplete context, explicit exclusions, spacing, or runtime caps. `evidence.repeatedBumperMatches` reports supporting occurrences. No timestamps, show names, or stored logo images are hardcoded into detection.

Default runtime caps remain three below 30 minutes and five below 60 minutes, then allow eight below 90 minutes, ten below 120 minutes, and twelve for longer supported files. Caps remain configurable upper bounds and never create candidates.

`conservative-fade-v7` accommodates short pauses after repeated bumpers: black duration must meet the configured minimum (and be at least 250 ms), while overlapping silence must cover 75% of that interval, capped at an 800 ms requirement and never below `minSilenceMs`. This replaces v6's fixed 500 ms black / 800 ms silence eligibility floors. The fade, animated nine-frame matching sequence, three independent occurrences, and other non-context safeguards remain unchanged. The separate one-second silence requirement for black intervals longer than two seconds still applies.
